...
#Started adding my code here




#CircuitScoreClass - create a class to store the circuit data with paramters in
# i loaded numpy in to do this but pycharme (my IDE) yelled at me and had me take it out

class Circuit: #each circuit probably needs a  unique ID right?
    def_init_(res.circuit_score, ymin_lac, alpha_lac, beta_lac, ymax_tet, ymin_tet, alpha_tet, beta_tet,
              ymax_ara, ymin_ara, alpha_ara, beta_ara, ymax_lux, ymin_lux, alpha_lux, beta_lux)
    variable =



# STORE RESULTS IN AN ARRAY - calling it score chart
index = 0
scoreChart: object = np.array(index + 1, res2.circuit_score, ) #append the other data
print(scoreChart)
#plan was to create an array with an index value, circuit score, and other details
# on the circuit as an object (maybe these 2 can't be combined??) and then  with the data stored we
# can manipulate and take next steps or at a minimum  select the best one after running it 100+ times